...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'Casablanca-v' + version + '-windows' + archSuffix + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'Casablanca-v' + version)
  archive.pipe(output)
  ...
  const options = {
    src: packagePath,
    dest: 'dist/app/casablanca-installer' + archSuffix,
  ...
  await installer(options)
    .then(function () {
      fs.renameSync('./dist/app/casablanca-installer' + archSuffix + '/casablanca-' + version + '-setup.exe', './dist/app/casablanca-' + version + archSuffix + '-setup.exe')
    })
    .catch(err => {
