...
// Restore Hardware Acceleration (Vital for Video Decoding)
// app.disableHardwareAcceleration() // REMOVED
...
try {
  ...
  console.warn('crashReporter not available', e)
}

if (process.argv.some(arg => arg === '-v' || arg === '--version')) {
  console.log('Casablanca: ' + app.getVersion())
...
app.commandLine.appendSwitch('disable-features', 'TrustedTypes,HardwareMediaKeyHandling,MediaSessionService')
...
function createWindow(customArgs = {}) {
  ...
  // (since the screen Casablanca was previously open on could have been removed)
...
app.on('ready', function () {
  ...
  getWindowWebContents(newWin).on('did-finish-load', function () {
    // if a URL was passed as a command line argument (probably because Casablanca is set as the default browser on Linux), open it.
  ...
  if (process.platform === 'win32') {
    // Hide the native application menu on Windows;
    // users access actions from the in-app (three-dots) menu instead.
...
ipc.on('places-connect', function (e) {
  const win = ensurePlacesWindow()

  if (win.webContents.isLoadingMainFrame()) {
    win.webContents.once('did-finish-load', function () {
      if (!win.isDestroyed()) {
        win.webContents.postMessage('places-connect', null, e.ports)
      }
    })
