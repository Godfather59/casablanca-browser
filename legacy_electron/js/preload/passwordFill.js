...
function addFocusListener (element, credentials) {
  ...
  function addOption (parent, username) {
    ...
    suggestionItem.style = 'padding: 10px; cursor: pointer; background-color: #fff; border-bottom: 1px solid #d4d4d4; font-size: 16px;'
...
function maybeAddUnlockButton (target) {
  // Unlock button (key icon) disabled; we now trigger
  // autofill automatically from the renderer instead of
  // showing a clickable key overlay.
}
...
function handleBlur (event) {
  // no-op now that the unlock button is disabled
}
...
ipc.on('password-autofill-enabled', (event) => {
  ...
  // Also request autofill immediately so suggestions appear
  // as soon as the login fields are focused, without clicking
  // the key icon.
  requestAutofill()
})
