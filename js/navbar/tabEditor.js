...
const tabEditor = {
  ...
  show: function (tabId, editingValue, showSearchbar) {
    ...
    // When showSearchbar is explicitly false (e.g. clicking the URL bar),
    // we only enter edit mode and select the text. The search suggestions
    // overlay is not shown until the user starts typing.
  ...
  initialize: function () {
    ...
    tabEditor.input.addEventListener('input', function (e) {
      ...
      // If the searchbar overlay isn't visible yet (for example when the
      // user clicked the URL bar and we only selected the text), show it
      // the first time the user types.
      if (searchbar.el.hidden || searchbar.associatedInput !== tabEditor.input) {
        searchbar.show(tabEditor.input)
      }
